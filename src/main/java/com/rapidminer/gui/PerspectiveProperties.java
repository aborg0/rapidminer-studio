		...
		if (mainFrame != null) {
			...
			for (DockableState state : states) {
				...
				if (dockable.getComponent() instanceof Container) {
					...
					if (scrollPane != null) {
						ScrollBarsPosition scrollBarsPosition = new ScrollBarsPosition(scrollPane.getVerticalScrollBar()
								.getValue(), scrollPane.getHorizontalScrollBar().getValue());
						...
						public void run() {
							...
							do {
								...
								i++;
									...
									&& (vScrollBar.getValue() != scrollBarsPosition.getValue().getVertical() || hScrollBar
											.getValue() != scrollBarsPosition.getValue().getHorizontal()));
						}
					}).start();
				}
			}
