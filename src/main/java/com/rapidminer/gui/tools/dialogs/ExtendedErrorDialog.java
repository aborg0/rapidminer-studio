		...
		if (useBugzilla && isBug) {
			sendReport = new JButton(new ResourceAction("send_bugreport") {
				...
				public void actionPerformed(ActionEvent e) {
					...
					// final char[] pw = new char[] { '!', 'z', '4', '8', '#',
					// 'H', 'c', '2', '$', '%', 'm', ')',
					// '9', '+', '*', '*' };
					// final String email = "bugs@rapid-i.com";
					// try {
					// final XmlRpcClient rpcClient =
					// XmlRpcHandler.login(XmlRpcHandler.BUGZILLA_URL, email,
					// pw);
					...
					// @Override
					// public void run() {
					// // if operator from import group is
					// // present, ask the user to
					// // include the data in the bug report
					// if (RapidMinerGUI.getMainFrame() != null
					// && RapidMinerGUI.getMainFrame().getProcess() != null) {
					// for (final Operator op :
					// RapidMinerGUI.getMainFrame().getProcess()
					// .getAllOperators()) {
					// if
					// (op.getOperatorDescription().getGroup().toLowerCase(Locale.ENGLISH)
					// .contains("import")
					// || op.getName().toLowerCase(Locale.ENGLISH)
					// .equals("retrieve")) {
					// SwingTools.showMessageDialog(
