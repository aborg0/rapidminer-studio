 ...
 * @author Gabor Bakos
 */
public abstract class AbstractUIState implements MainUIState, ProcessEndHandler, WindowListener {
	/**
...
//	public final transient Action ANOVA_CALCULATOR_ACTION = new AnovaCalculatorAction();
	public final transient Action MANAGE_CONFIGURABLES_ACTION = new ManageConfigurablesAction()/*new ResourceAction(true, "manage_configurables") {
		...
		if (SystemInfoUtilities.getOperatingSystem() == OperatingSystem.OSX) {
			...
			try {
				OSXQuitListener quitListener = new OSXQuitListener() {
					...
					@Override
					public void quit() {
					    AbstractUIState.this.exit(false);
					}
				};
				if (frame != null) {
					OSXAdapter.adaptUI(frame, SETTINGS_ACTION, new AboutAction(), quitListener);
				}
			} catch (Throwable t) {
...
//	}
	...
	public Action getRestorePerspectiveAction() {
		return RESTORE_PERSPECTIVE_ACTION;
	}
...
//	}
	...
	public Action getProcessImport() {
		return IMPORT_PROCESS_ACTION;
	}

	@Override
	public Action getProcessExport() {
		return EXPORT_PROCESS_ACTION;
	}
	...
	public Action getNewPerspective() {
		return NEW_PERSPECTIVE_ACTION;
	}

	@Override
	public Action getManageConfigurables() {
		return MANAGE_CONFIGURABLES_ACTION;
	}
	...
	public Action getBrowseVideos() {
		return BROWSE_VIDEOS_ACTION;
	}

	@Override
	public Action getBrowseDocumentation() {
		return BROWSE_DOCUMENTATION_ACTION;
	}

	@Override
	public Action getBrowseCommunity() {
		return BROWSE_COMMUNITY_ACTION;
	}

	@Override
	public Action getBrowseSupport() {
		return BROWSE_SUPPORT_ACTION;
	}
