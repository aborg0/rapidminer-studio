...
public abstract class BubbleWindow extends JDialog {
	...
	protected BubbleWindow(final Window owner, final BubbleStyle style, final AlignedSide preferredAlignment,
		...
		if (!isPerPixelTranslucencySupported) {
			LogService.getRoot().log(Level.WARNING, "com.rapidminer.gui.tour.BubbleWindow.per_pixel_not_supported");
		}
	...
	protected void registerRegularListener() {
		if (!listenersAdded) {
			...
			if (window != null) {
				...
				window.addComponentListener(componentListenerToWindow);
			}
	...
	protected void unregisterRegularListener() {
		if (listenersAdded) {
			...
			if (window != null) {
				...
				window.removeComponentListener(componentListenerToWindow);
			}
			if (preferredAlignment == AlignedSide.MIDDLE) {
				if (addPerspective) {
	...
	protected void changeToAssistant(final AssistantType type) {
		...
		switch (type) {
			...
			case WRONG_PERSPECTIVE:
				...
				} else if (myPerspective.equals("welcome")) {
				...
				RapidMinerGUI.getMainFrame().getPerspectives().addPerspectiveChangeListener(assistantPerspective);
				break;
			case NOT_IN_CHAIN:
	...
	private void closePerspectiveAssistant() {
		if (assistantBubble != null && currentAssistant == AssistantType.WRONG_PERSPECTIVE) {
			...
			assistantBubble = null;
			RapidMinerGUI.getMainFrame().getPerspectives().removePerspectiveChangeListener(assistantPerspective);
			currentAssistant = AssistantType.NO_ASSISTANT_ACTIVE;
		}
	}
